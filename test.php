...
if($_SESSION['hi']==1)
{
	...
	$nxt1=$_SESSION['nxt'];
	$set=$_SESSION['set'];
	$sqln="INSERT INTO tmarkfinal VALUES ($regno,'ques' , '$scode','$tcode', '$set',";
	$i=1;
	foreach($qno as $q2) 
   { $sqln=$sqln."'".$q2."', ";
   ...
   for($j=$i;$j<=50;$j++)
   {
	...
	$sqln="INSERT INTO tmarkfinal VALUES ($regno,'ans' , '$scode','$tcode', '$set', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '',0,60,'pending') ";
	$l=$s->linkarivu();
	$result = mysqli_query($l,$sqln);  
	if (!$result) {
    // echo "Checking... 2......";
...
if ($row= mysqli_fetch_assoc($result)) 
 { 
    if($set=='s4') 
   {$filename1='picts2/'.$scode.'e'.$tcode.'qq'.$qn.'q.jpg';
   $filename2='picts2/'.$scode.'e'.$tcode.'qq'.$qn.'a1.jpg';
 $filename3='picts2/'.$scode.'e'.$tcode.'qq'.$qn.'a2.jpg';
 $filename4='picts2/'.$scode.'e'.$tcode.'qq'.$qn.'a3.jpg';
 $filename5='picts2/'.$scode.'e'.$tcode.'qq'.$qn.'a4.jpg';
   }
   else
   {
